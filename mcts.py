...
class MCTS:
    def __init__(self, root,simulation,t,exploration_weight=math.sqrt(2)):
        self.__root = root
    ...
    def expansion(self,cur):
        ...
        for i in moves:
            state=State(cur,False,None,cp=-1*cur.get_player(),arow=i[0],acul=i[1])
    ...
    def simulation1(self,cur):
        ...
        while result == "is not terminal":
            ...
            result=state.evaluation(cp=cur.get_player())
        if result=="win":
            return 1
        elif result=="loss":
            return -1
